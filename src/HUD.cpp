...
HUD::~HUD()
{ // FIXME sgmentation fault when closing program. Maybe we don't need this implementation
    // deleting quickslot
    for (int i = 0; i < 3; i++)
        delete quickSlot[i];
...
{ //, bool isInteracting
    // handling inputs
    // opens inventory
    if (keyInput.type == sf::Event::KeyPressed && keyInput.key.code == sf::Keyboard::E && !isInteraction)
    ...
    if (isInvOpen)
    {
        ...
        if (keyInput.type == sf::Event::KeyPressed && keyInput.key.code == sf::Keyboard::Up && !quickAssign)
        { // when quickAssign is open, it will not scroll
        ...
        if (keyInput.type == sf::Event::KeyPressed && keyInput.key.code == sf::Keyboard::Down && !quickAssign)
        { // when quickAssign is open, it will not scroll
...
{
    if (npc.isInteractable(190, hero))
    {
        NPCAggro = true;
    }
    else
